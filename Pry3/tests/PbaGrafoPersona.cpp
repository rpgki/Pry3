...
void testOperator(){
    ...
    vrtPba = grafo[idVrt];
    if (vrtPba.obtNombre() != "Carlos" && vrtPba.obtEdad() != 27 && vrtPba.obtId() != "113980914" && vrtPba.obtSexo() != 'M') {
...
void testAsgAdy() {
    ...
    for(int i = 0; i < vecAdy.size(); i++){
        if (vecAdy[i].obtNombre() == idVrtD.obtNombre() && vecAdy[i].obtApellido1() == idVrtD.obtApellido1() && vecAdy[i].obtApellido2() == idVrtD.obtApellido2())
            rsl = true;
        else
            rsl = false;
    }
...
void testElmVrt() {
    Nombre idVrt("FabNombreiola", "Salazar","Artavia");
...
void testElmAdy() {
    Persona vrtO("Fabiola","123456789",18,'F');
    Persona vrtD("Luis","123456789",18,'M');
    Nombre idVrtO;
    Nombre idVrtD;
    Grafo<Nombre,Persona> grafo;
    grafo.asgVrt(idVrtO,vrtO);
    grafo.asgVrt(idVrtD, vrtD);
    grafo.asgAdy(idVrtO, idVrtD);
